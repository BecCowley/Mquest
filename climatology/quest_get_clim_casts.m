...
tcor = -i*2*pi/366;
if(ispc)
    cpath = [CLIMATOLOGY_PATH_PC];
else
    cpath = [CLIMATOLOGY_PATH_UNIX];
end

cfnm = clname(prop,cpath,fname);
fn = [cfnm '.nc'];
gor = ncread(fn,'gr_origin');
gsp = ncread(fn,'grid_space');
rot = ncread(fn,'rotation');
cnrs = ncread(fn,'corners');
...
if ~isempty(ic)
    ...
    if isempty(doy)
        ...
        X = X+1-min(ix);
        Y = Y+1-min(iy);
        mt = zeros(ndep,length(ic));at = mt;st = mt;
        if ~nomean
            for ii = 1:ndep
                mt(ii,:) = interp2(mn(:,:,ii),X,Y,'*linear');
            end
        end
        if tdep>0
            for ii = 1:size(an,3)
                at(ii,:) = interp2(an(:,:,ii),X,Y,'*linear');
            end
            if semian
                for ii = 1:size(sa,3)
                    st(ii,:) = interp2(sa(:,:,ii),X,Y,'*linear');
                end
            end
        end
...
if woa>0 & ~isempty(out)
   ...
   if woa==1
      vv(:,out) = quest_get_woa05_profiles(prop,lon(out),lat(out),deps,dok,vart);	   
   elseif woa==2
      wdep = dep_csl(deps,1);
	 ...
	 disp(['a smaller set on depth levels: ' num2str(nwdep)]);
      ...
      if ~isempty(iw)
	 vv(iw,out) = quest_get_woa_profiles(prop,lon(out),lat(out),wdep(iw),dok,vart);
      end
   end
   out = [];      
end   
